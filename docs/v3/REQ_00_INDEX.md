# REQ_00_INDEX：OpenQA 需求文档索引

本文档汇总 OpenQA v3 的需求拆分，来源于 `PRODUCT.md`。

| 文档 | 主题 | 说明 |
| --- | --- | --- |
| `REQ_01_COMMANDS_AND_INIT.md` | 指令与初始化 | 用户指令（`init/run/apply/archive/update`）与内部命令（`_advance`）分层设计、AI 驱动循环协议、`/oqa:*` Slash Commands、初始化体验、执行过程可见性、调试命令。 |
| `REQ_02_WORKSPACE_AND_ARTIFACTS.md` | 工作区与产物 | `openqa/` 完整目录结构（唯一权威定义）、产物格式选择原则、AI 宿主隐藏目录、`config.yaml` 画像示例。 |
...
| `REQ_12_OPENSPEC_INTEGRATION.md` | OpenSpec 联动 | OpenSpec change 与 OpenQA change 的联动工作流和产物关系。 |
