...
OpenGuard 以 change 作为迭代单位，把一次需求更新、代码修改、缺陷修复、测试策略调整纳入同一个闭环。change 工作区本身必须适合 Agent 宿主直接读取；上下文组织由宿主负责，OpenGuard 不额外生成专用 context 文件。

**核心设计：AI 驱动循环**

`openguard run` 完成初始化扫描后，通过 `state.yaml` 的 `agent_action` 字段驱动 AI 自主推进，直到到达需要用户确认的门槛点。CLI 工具（`openguard _advance`）负责确定性操作，AI 负责需要理解/推理的操作，两者通过文件和 `state.yaml` 协作。
...
| 编号 | 需求 |
| --- | --- |
| REQ-03-01 | `openguard run <目标>` 必须创建 `openguard/changes/<change-id>/`，执行初始扫描，并将 `state.yaml` 的 `agent_action` 设为 `generate`，触发 AI 驱动循环。 |
| REQ-03-02 | change 必须记录目标、来源需求、相关代码变更、人工备注和当前状态。 |
| REQ-03-03 | `openguard _advance` 必须读取 `state.yaml` 并自动执行确定性产物生成（scan/impact/matrix），更新 `agent_action` 反映下一步行动类型。 |
| REQ-03-04 | `openguard apply` 必须只在 `phase: ready_for_apply` 且必要产物新鲜时运行。 |
| REQ-03-05 | `openguard archive` 必须在产物完整、报告处理完毕后归档 change。 |
| REQ-03-06 | 未解决 unknowns、blocking Review、失败门禁不得被静默跳过。 |
| REQ-03-07 | `openguard archive` 必须输出可晋升的测试脚本列表，由 Agent 或人工确认后晋升到 `openguard/test_assets/` 并建立锚点文件。 |
| REQ-03-08 | 归档时必须同步更新受影响的 `openguard/suites/` 套件定义。 |
| REQ-03-09 | `openguard run` 时必须对目标模块执行 `knowledge-scan`；触发条件、提取内容和锚点规则见 `REQ_04_SCAN_AND_IMPACT.md`。 |
| REQ-03-10 | `openguard run` 时必须查询 `preconditions.yaml`，为当前 change 的外部前置需求匹配已有路径；外部前置标签由 AI 在 `test_knowledge.md` 的 `external_preconditions` 字段中填写，CLI 不做关键词推断。 |
| REQ-03-11 | `openguard run --from-openspec` 时必须对 OpenSpec 内容做有效性校验，并将结果写入 `openspec_link.yaml`。 |

## change 工作区

change 工作区位于 `openguard/changes/<change-id>/`，完整目录结构见 `REQ_02_WORKSPACE_AND_ARTIFACTS.md`。
...
next_cli: "openguard _advance"  # AI 下一步应调用的命令（明确不歧义）
...
| 情况 | `next_cli` |
| --- | --- |
| 有缺失产物需 AI 生成 | `"openguard _advance"` （AI 生成后调此命令打卡） |
| 产物已生成，需继续扫描 | `"openguard _advance"` |
| 准备就绪，等用户确认 | `null`（wait_user，不需要 CLI） |
| 用户确认后 | `"openguard apply"` |
...
| `agent_action: run_cli` | AI | 调用 `openguard _advance` | 触发影响分析 |
...
- change 可暂停、恢复、归档。
- `state.yaml` 的 `agent_action` 必须在每次 `openguard _advance` 执行后正确更新。
- AI 生成产物后调用 `openguard _advance`，CLI 验证产物存在并更新状态，不做内容解读。
