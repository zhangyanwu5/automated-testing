...
def qa_workspace(tmp_path: Path) -> tuple[Path, Path]:
    """返回 (project_root, openqa_dir)，已初始化好目录结构。"""
...
class TestAnchorStateMachine:
    def _make_meta(self, openqa_dir: Path, script_name: str, status: str = "verified",
    ...
    def test_symbol_hash_change_triggers_needs_review(self, qa_workspace):
        ...
        _, openqa_dir = qa_workspace
        self._make_meta(openqa_dir, "login.py", status="verified",
                        symbols=[{"path": "Assets/Login.cs", "symbol": "Login.Handle", "hash": "oldHash"}])

        from openqa.assets.lifecycle import check_anchor_freshness
    ...
    def test_symbol_missing_triggers_stale(self, qa_workspace):
        ...
        from openqa.assets.lifecycle import check_anchor_freshness
        # 不传入该符号 → 视为消失
        reports = check_anchor_freshness(openqa_dir, current_symbols={})
    ...
    def test_stale_symbol_missing_triggers_broken(self, qa_workspace):
        ...
        _, openqa_dir = qa_workspace
        self._make_meta(openqa_dir, "login.py", status="stale",
    ...
    def test_verified_script_allowed_in_all_gates(self, qa_workspace):
        ...
        self._make_meta(openqa_dir, "login.py", status="verified")
        # 手动写 suite.yaml
        suite_dir = openqa_dir / "suites" / "smoke"
        suite_dir.mkdir(parents=True, exist_ok=True)
        (suite_dir / "suite.yaml").write_text(
            yaml.dump({"name": "smoke", "scripts": [{"path": "test_assets/scripts/login.py"}]}),
            encoding="utf-8",
        )
        from openqa.assets.lifecycle import list_suite_scripts
        for gate in ("local", "ci", "release", "nightly"):
            scripts = list_suite_scripts(openqa_dir, "smoke", gate)
    ...
    def test_needs_review_blocked_in_release_gate(self, qa_workspace):
        ...
        _, openqa_dir = qa_workspace
        self._make_meta(openqa_dir, "login.py", status="needs-review")
        suite_dir = openqa_dir / "suites" / "smoke"
        suite_dir.mkdir(parents=True, exist_ok=True)
        (suite_dir / "suite.yaml").write_text(
            yaml.dump({"name": "smoke", "scripts": [{"path": "test_assets/scripts/login.py"}]}),
        ...
        from openqa.assets.lifecycle import list_suite_scripts
        scripts = list_suite_scripts(openqa_dir, "smoke", "release")
    ...
    def test_broken_script_skipped_in_all_gates(self, qa_workspace):
        ...
        _, openqa_dir = qa_workspace
        self._make_meta(openqa_dir, "login.py", status="broken")
        suite_dir = openqa_dir / "suites" / "smoke"
        suite_dir.mkdir(parents=True, exist_ok=True)
        (suite_dir / "suite.yaml").write_text(
            yaml.dump({"name": "smoke", "scripts": [{"path": "test_assets/scripts/login.py"}]}),
...
class TestSuiteManagement:
    def test_add_script_to_suite(self, qa_workspace):
        ...
        _, openqa_dir = qa_workspace
        from openqa.assets.lifecycle import add_script_to_suite, load_suite
        result = add_script_to_suite(
            openqa_dir, "smoke", "test_assets/scripts/login.py"
        )
        assert result is True
        suite = load_suite(openqa_dir / "suites" / "smoke" / "suite.yaml")
    ...
    def test_suite_rejects_change_dir_script(self, qa_workspace):
        ...
        _, openqa_dir = qa_workspace
        from openqa.assets.lifecycle import add_script_to_suite
        result = add_script_to_suite(
            openqa_dir, "smoke", "openqa/changes/chg-001/test_scripts/login.py"
        )
    ...
    def test_add_script_deduplication(self, qa_workspace):
        ...
        _, openqa_dir = qa_workspace
        from openqa.assets.lifecycle import add_script_to_suite, load_suite
        add_script_to_suite(openqa_dir, "smoke", "test_assets/scripts/login.py")
        add_script_to_suite(openqa_dir, "smoke", "test_assets/scripts/login.py")
        suite = load_suite(openqa_dir / "suites" / "smoke" / "suite.yaml")
    ...
    def test_requirement_suite_subpath(self, qa_workspace):
        ...
        from openqa.assets.lifecycle import add_script_to_suite
        result = add_script_to_suite(
            openqa_dir, "requirement/REQ-001", "test_assets/scripts/req001.py"
        )
        assert result is True
        assert (openqa_dir / "suites" / "requirement" / "REQ-001" / "suite.yaml").exists()
    ...
    def test_suite_execution_manifest_has_anchor_hash(self, qa_workspace):
        ...
        from openqa.assets.lifecycle import build_suite_execution_manifest
        manifest = build_suite_execution_manifest(openqa_dir, "smoke", "local", code_version="v1.2.3")
...
class TestScriptPromotion:
    ...
    def test_promote_rejects_without_run_ids(self, qa_workspace, tmp_path):
        ...
        from openqa.knowledge.manager import promote_script
        result = promote_script(draft, openqa_dir, run_ids=None)
    ...
    def test_promote_adds_to_suite_yaml(self, qa_workspace, tmp_path):
        ...
        from openqa.knowledge.manager import promote_script
        from openqa.assets.lifecycle import load_suite
        promote_script(
            draft, openqa_dir,
            change_id="chg-001",
            run_ids=["run-1"],
    ...
    def test_meta_file_same_stem_as_script(self, qa_workspace, tmp_path):
        ...
        from openqa.knowledge.manager import promote_script
        result = promote_script(draft, openqa_dir, run_ids=["r1"], change_id="c1")
...
class TestIntrusionStrategy:
    def test_external_only_always_ok(self):
        ...
        from openqa.assets.lifecycle import check_intrusion_strategy
    ...
    def test_intrusion_strategy_in_preflight(self, qa_workspace):
        ...
        result = pre_apply_check(None, openqa_dir, config, gate="local")
    ...
    def test_intrusion_strategy_changed_detected(self, qa_workspace):
        ...
        _, openqa_dir = qa_workspace
        from openqa.assets.lifecycle import check_intrusion_strategy_changed
        config1 = {"runtime": {"intrusion_strategy": "external-only"}}
        config2 = {"runtime": {"intrusion_strategy": "build-time-bridge"}}
        # 首次记录
        check_intrusion_strategy_changed(openqa_dir, config1)
        # 变化
        changed = check_intrusion_strategy_changed(openqa_dir, config2)
...
class TestOpenSpecDetection:
    ...
    def test_detect_no_openspec_no_effect_on_init(self, qa_workspace):
        """未安装 OpenSpec 时，OpenQA init 工作流不受影响（REQ-12 验收）。"""
...
class TestOpenSpecLink:
    def test_write_openspec_link(self, openspec_project, tmp_path):
        ...
        from openqa.openspec.detector import write_openspec_link, read_openspec_link
...
class TestOpenSpecChangedAndClearance:
    def test_no_change_when_content_same(self, openspec_project, tmp_path):
        ...
        from openqa.openspec.detector import write_openspec_link, check_openspec_changed
        change_dir = tmp_path / "chg-001"
        change_dir.mkdir()
        write_openspec_link(change_dir, "opsx-001", openspec_project)
...
class TestNewFromOpenSpec:
    def test_new_from_openspec_creates_link(self, openspec_project, monkeypatch):
        """REQ-12-02/04：openqa new --from-openspec 生成 openspec_link.yaml。"""
        import argparse
    ...
    def test_new_from_openspec_state_records_link(self, openspec_project, monkeypatch):
        ...
        openqa_dir = openspec_project / "openqa"
        change_dir = next((openqa_dir / "changes").iterdir())
