...
def change_with_impact(tmp_path: Path):
    ...
    _, change_dir = create_change(openqa_dir, "验证认证逻辑")
...
class TestReviewPlan:
    def test_review_plan_created(self, change_with_impact):
        from openqa.review.planner import generate_review_plan
        _, openqa_dir, change_dir, config = change_with_impact
        path = generate_review_plan(change_dir, openqa_dir, config)
        assert path.exists()
    ...
    def test_review_level_off_returns_empty_scope(self, change_with_impact):
        from openqa.review.planner import generate_review_plan
        _, openqa_dir, change_dir, config = change_with_impact
        path = generate_review_plan(change_dir, openqa_dir, config, review_level="off")
        content = path.read_text(encoding="utf-8")
...
class TestSarifFindings:
    def test_sarif_skeleton_created(self, change_with_impact):
        ...
        from openqa.review.findings import build_sarif_skeleton, write_sarif
        _, openqa_dir, change_dir, config = change_with_impact
    ...
    def test_sarif_is_valid_schema(self, change_with_impact):
        from openqa.review.findings import build_sarif_skeleton
        _, openqa_dir, change_dir, config = change_with_impact
        sarif = build_sarif_skeleton(change_dir, config)
    ...
    def test_make_finding_has_required_fields(self):
        ...
        from openqa.review.findings import make_finding
        f = make_finding(
            "OQA006", "error", "src/auth.py", 10,
            "密码未加密存储",
    ...
    def test_blocking_finding_detected(self, change_with_impact):
        ...
        from openqa.review.findings import build_sarif_skeleton, write_sarif, make_finding, has_blocking_findings
        _, openqa_dir, change_dir, config = change_with_impact

        blocking = make_finding("OQA006", "error", "src/auth.py", 1,
    ...
    def test_no_blocking_finding(self, change_with_impact):
        from openqa.review.findings import build_sarif_skeleton, write_sarif, has_blocking_findings
        _, openqa_dir, change_dir, config = change_with_impact
...
class TestReviewReport:
    def test_review_report_created(self, change_with_impact):
        ...
        _, openqa_dir, change_dir, config = change_with_impact
        sarif_path, report_path = run_review_generation(change_dir, openqa_dir, config)
    ...
    def test_review_report_mentions_no_code_change(self, change_with_impact):
        ...
        from openqa.review.findings import run_review_generation
        _, openqa_dir, change_dir, config = change_with_impact
        _, report_path = run_review_generation(change_dir, openqa_dir, config)
        content = report_path.read_text(encoding="utf-8")
        assert "REQ-06-07" in content or "不直接修改" in content

    def test_review_report_blocking_summary(self, change_with_impact):
        from openqa.review.findings import run_review_generation, make_finding
        _, openqa_dir, change_dir, config = change_with_impact
        findings = [make_finding("OQA006", "error", "src/auth.py", 1, "安全", is_blocking=True)]
        _, report_path = run_review_generation(change_dir, openqa_dir, config, findings)
        content = report_path.read_text(encoding="utf-8")
        assert "blocking" in content.lower() or "阻断" in content
