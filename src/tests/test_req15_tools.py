...
class TestRegistry:
    def test_tool_result_ok(self):
        from openqa.tools.registry import ToolResult
    ...
    def test_list_tools_returns_metas(self):
        from openqa.tools import list_tools
        tools = list_tools()
        assert len(tools) > 0
        for m in tools:
            assert m.name
    ...
    def test_call_tool_not_found(self):
        from openqa.tools import call_tool
...
class TestFsTools:
    def test_read_text_ok(self, tmp_path):
        from openqa.tools.fs.read import read_text
        f = tmp_path / "a.txt"
    ...
    def test_read_yaml_ok(self, tmp_path):
        from openqa.tools.fs.read import read_yaml
        f = tmp_path / "cfg.yaml"
    ...
    def test_read_json_ok(self, tmp_path):
        from openqa.tools.fs.read import read_json
    ...
    def test_content_hash(self):
        from openqa.tools.fs.hash import content_hash
    ...
    def test_load_ignore(self, tmp_path):
        from openqa.tools.fs.ignore import load_ignore
        openqa_dir = tmp_path / "openqa"
        openqa_dir.mkdir()
        (openqa_dir / "ignore").write_text("*.pyc\n# comment\n__pycache__/\n", encoding="utf-8")
        patterns = load_ignore(openqa_dir)
    ...
    def test_find_files_basic(self, tmp_path):
        from openqa.tools.fs.search import find_files
    ...
    def test_find_executables_depth_limit(self, tmp_path):
        from openqa.tools.fs.search import find_executables
...
class TestRuntimeTools:
    def test_check_executable_python(self):
        from openqa.tools.runtime.process import check_executable
        import sys
    ...
    def test_run_command_success(self):
        from openqa.tools.runtime.process import run_command
    ...
    def test_get_env(self, monkeypatch):
        from openqa.tools.runtime.process import get_env
        monkeypatch.setenv("TEST_OPENQA_KEY", "test_value_42")
        assert get_env("TEST_OPENQA_KEY") == "test_value_42"
    ...
    def test_find_unity_editors_returns_list(self, tmp_path):
        ...
        from openqa.tools.runtime.unity import find_unity_editors
...
class TestAnalysisTools:
    def test_detect_language_known(self, tmp_path):
        from openqa.tools.analysis.symbols import detect_language
    ...
    def test_extract_symbols_python(self, tmp_path):
        from openqa.tools.analysis.symbols import extract_symbols
        f = tmp_path / "mod.py"
