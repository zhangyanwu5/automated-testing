...
class TestEnsureRunDir:
    def test_suite_run_dir(self, initialized_project: Path):
        from openqa.workspace.layout import ensure_run_dir
        openqa_dir = initialized_project / "openqa"
        run_dir = ensure_run_dir(openqa_dir, "run-20260501T100000Z", suite_name="smoke")
    ...
    def test_change_run_dir(self, initialized_project: Path):
        from openqa.workspace.layout import ensure_run_dir
        openqa_dir = initialized_project / "openqa"
        run_dir = ensure_run_dir(openqa_dir, "run-20260501T100000Z", change_id="chg-001")
        assert run_dir.is_dir()
    ...
    def test_run_dir_is_independent(self, initialized_project: Path):
        ...
        from openqa.workspace.layout import ensure_run_dir
        openqa_dir = initialized_project / "openqa"
        r1 = ensure_run_dir(openqa_dir, "run-001", suite_name="smoke")
        r2 = ensure_run_dir(openqa_dir, "run-002", suite_name="smoke")
        assert r1 != r2
    ...
    def test_missing_both_raises(self, initialized_project: Path):
        ...
        with pytest.raises(ValueError):
            ensure_run_dir(openqa_dir, "run-001")
...
class TestReportsRetention:
    def test_config_has_reports_retention(self, initialized_project: Path):
        with (initialized_project / "openqa" / "config.yaml").open(encoding="utf-8") as f:
