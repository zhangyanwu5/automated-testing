...
def project_with_change(tmp_path: Path):
    ...
    _, change_dir = create_change(openqa_dir, "验证用户认证逻辑")
    return tmp_path, openqa_dir, change_dir
...
class TestIgnoreRules:
    def test_load_ignore_patterns(self, project_with_change):
        from openqa.scan.scanner import load_ignore_patterns
        _, openqa_dir, _ = project_with_change
        patterns = load_ignore_patterns(openqa_dir)
        assert len(patterns) > 0
        assert ".git/" in patterns

    def test_openqa_dir_excluded(self, project_with_change):
        from openqa.scan.scanner import load_ignore_patterns, _is_ignored
        tmp_path, openqa_dir, _ = project_with_change
        patterns = load_ignore_patterns(openqa_dir)
        # openqa/ 内文件应被忽略
        test_path = openqa_dir / "config.yaml"
...
class TestFileScanning:
    def test_full_scan_returns_files(self, project_with_change):
        from openqa.scan.scanner import run_full_scan
        from openqa.init.config_writer import load_config
        tmp_path, openqa_dir, _ = project_with_change
        config = load_config(openqa_dir)
        result = run_full_scan(tmp_path, openqa_dir, config)
    ...
    def test_analyzer_version_recorded(self, project_with_change):
        from openqa.scan.scanner import run_full_scan, ANALYZER_VERSION
        from openqa.init.config_writer import load_config
        tmp_path, openqa_dir, _ = project_with_change
        config = load_config(openqa_dir)
        result = run_full_scan(tmp_path, openqa_dir, config)
...
class TestSnapshotWrite:
    def test_execute_scan_creates_snapshot(self, project_with_change):
        ...
        from openqa.init.config_writer import load_config
        _, openqa_dir, change_dir = project_with_change
        config = load_config(openqa_dir)
        snap_path, _ = execute_scan(change_dir, openqa_dir, config, "full")
    ...
    def test_full_scan_creates_full_index(self, project_with_change):
        ...
        full_index = openqa_dir / "artifacts" / "full_index.json"
    ...
    def test_incremental_scan_creates_delta(self, project_with_change):
        ...
        execute_scan(change_dir, openqa_dir, config, "full")
        # 再执行增量
        _, delta_path = execute_scan(change_dir, openqa_dir, config, "incremental")
...
class TestFreshnessCheck:
    ...
    def test_freshness_passes_after_scan(self, project_with_change):
        ...
        config = load_config(openqa_dir)
        execute_scan(change_dir, openqa_dir, config, "full")
        is_fresh, path = run_freshness_check(change_dir, openqa_dir)
    ...
    def test_freshness_json_written(self, project_with_change):
        from openqa.scan.freshness import run_freshness_check
        _, openqa_dir, change_dir = project_with_change
        _, path = run_freshness_check(change_dir, openqa_dir)
...
class TestImpactGraph:
    ...
    def test_impact_graph_created(self, project_with_change):
        ...
        openqa_dir, change_dir, config = self._scan_first(project_with_change)
    ...
    def test_impact_graph_has_schema_version(self, project_with_change):
        ...
        path = run_impact_analysis(change_dir, openqa_dir, config)
        data = json.loads(path.read_text(encoding="utf-8"))
        assert data["schema_version"].startswith("openqa/impact_graph/")
...
class TestMatrixGeneration:
    ...
    def test_matrix_generated(self, project_with_change):
        ...
        openqa_dir, change_dir, config = self._setup(project_with_change)
        path = run_matrix_generation(change_dir, openqa_dir, config)
        assert path.exists()
    ...
    def test_matrix_schema_version(self, project_with_change):
        ...
        path = run_matrix_generation(change_dir, openqa_dir, config)
        data = json.loads(path.read_text(encoding="utf-8"))
        assert data["schema_version"].startswith("openqa/test_matrix/")
    ...
    def test_matrix_blocked_when_runtime_incomplete(self, project_with_change):
        ...
        matrix = build_test_matrix(change_dir, openqa_dir, config, "smoke")
    ...
    def test_matrix_suite_full_no_fail_fast(self, project_with_change):
        ...
        _, openqa_dir, change_dir = project_with_change
        config = load_config(openqa_dir)
        matrix = build_test_matrix(change_dir, openqa_dir, config, "full")
