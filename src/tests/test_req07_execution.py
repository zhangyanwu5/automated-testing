...
def apply_env(tmp_path: Path):
    ...
    _, change_dir = create_change(openguard_dir, "验证主流程")
...
class TestRunId:
    ...
    def test_ensure_run_dir_creates_directory(self, apply_env):
        ...
        _, openguard_dir, change_dir, _ = apply_env
        run_id = make_run_id()
    ...
    def test_two_runs_independent_dirs(self, apply_env):
        ...
        from openguard.executor.run_id import ensure_run_dir
        _, openguard_dir, change_dir, _ = apply_env
        r1 = ensure_run_dir(openguard_dir, "run-001", change_id=change_dir.name)
        r2 = ensure_run_dir(openguard_dir, "run-002", change_id=change_dir.name)
        assert r1 != r2
...
class TestRunIndex:
    def test_suite_index_created(self, apply_env):
        ...
        from openguard.executor.run_id import update_suite_index
        _, openguard_dir, _, _ = apply_env
        update_suite_index(openguard_dir, "smoke", "run-001", "passed", 5, "abc123", "local")
        index_path = openguard_dir / "reports" / "suites" / "smoke" / "suite_index.yaml"
    ...
    def test_suite_index_latest_run_updated(self, apply_env):
        from openguard.executor.run_id import update_suite_index
        _, openguard_dir, _, _ = apply_env
        update_suite_index(openguard_dir, "smoke", "run-001", "passed", 5, "abc", "local")
        update_suite_index(openguard_dir, "smoke", "run-002", "failed", 5, "def", "ci")
        index_path = openguard_dir / "reports" / "suites" / "smoke" / "suite_index.yaml"
        with index_path.open(encoding="utf-8") as f:
            idx = yaml.safe_load(f)
    ...
    def test_change_run_index_created(self, apply_env):
        ...
        from openguard.executor.run_id import update_change_run_index
        _, openguard_dir, change_dir, _ = apply_env
        update_change_run_index(openguard_dir, change_dir.name, "run-001", "passed", 3, "abc", "local")
        idx_path = openguard_dir / "reports" / "changes" / change_dir.name / "change_run_index.yaml"
    ...
    def test_suite_index_retention(self, apply_env):
        ...
        for i in range(35):
            update_suite_index(openguard_dir, "smoke", f"run-{i:03d}", "passed", 1, "v", "local")
        idx_path = openguard_dir / "reports" / "suites" / "smoke" / "suite_index.yaml"
        with idx_path.open(encoding="utf-8") as f:
            idx = yaml.safe_load(f)
...
class TestPreflight:
    def test_preflight_fails_without_snapshot(self, apply_env):
        ...
        _, openguard_dir, change_dir, config = apply_env
        # 删除 snapshot
        snap = change_dir / "snapshot.json"
        snap.write_text('{"status": "pending"}', encoding="utf-8")
        result = pre_apply_check(change_dir, openguard_dir, config)
    ...
    def test_preflight_passes_after_full_setup(self, apply_env):
        ...
        _, openguard_dir, change_dir, config = apply_env
        run_freshness_check(change_dir, openguard_dir)
        result = pre_apply_check(change_dir, openguard_dir, config)
    ...
    def test_preflight_blocks_release_with_incomplete_runtime(self, apply_env):
        ...
        run_freshness_check(change_dir, openguard_dir)
        result = pre_apply_check(change_dir, openguard_dir, config, gate="release")
        assert result["is_blocked"] is True
...
class TestReporter:
    def _setup(self, apply_env):
        from openguard.executor.run_id import make_run_id, ensure_run_dir
        tmp_path, openguard_dir, change_dir, config = apply_env
        run_id = make_run_id()
        run_dir = ensure_run_dir(openguard_dir, run_id, change_id=change_dir.name)
        return openguard_dir, change_dir, config, run_id, run_dir

    def test_run_report_json_created(self, apply_env):
        from openguard.executor.reporter import build_run_report, write_run_report_json
        openguard_dir, change_dir, config, run_id, run_dir = self._setup(apply_env)
        ...
        assert data["schema_version"].startswith("openguard/run_report/")

    def test_run_report_junit_xml_created(self, apply_env):
        """REQ-07-05：必须生成 JUnit XML。"""
        from openguard.executor.reporter import build_run_report, write_run_report_junit
        openguard_dir, change_dir, config, run_id, run_dir = self._setup(apply_env)
        tasks = [{"task_id": "T-001", "name": "test", "type": "functional", "result": "passed"}]
        report = build_run_report(run_id, change_dir.name, None, tasks,
                                  run_dir=run_dir, config=config)
    ...
    def test_run_report_md_created(self, apply_env):
        ...
        from openguard.executor.reporter import build_run_report, write_run_report_md
        openguard_dir, change_dir, config, run_id, run_dir = self._setup(apply_env)
        tasks = [{"task_id": "T-001", "name": "test", "type": "functional", "result": "passed"}]
        report = build_run_report(run_id, change_dir.name, None, tasks,
                                  run_dir=run_dir, config=config)
    ...
    def test_timeline_md_created(self, apply_env):
        ...
        from openguard.executor.reporter import build_run_report, write_timeline
        openguard_dir, change_dir, config, run_id, run_dir = self._setup(apply_env)
        tasks = [{"task_id": "T-001", "name": "test", "type": "functional", "result": "passed"}]
        report = build_run_report(run_id, change_dir.name, None, tasks,
                                  run_dir=run_dir, config=config)
    ...
    def test_events_jsonl_written(self, apply_env):
        ...
        from openguard.executor.reporter import append_event
        from openguard.executor.run_id import ensure_run_dir, make_run_id
        tmp_path, openguard_dir, change_dir, config = apply_env
        run_id = make_run_id()
        run_dir = ensure_run_dir(openguard_dir, run_id, change_id=change_dir.name)
...
class TestApplyCommand:
    def test_apply_runs_and_produces_reports(self, apply_env, monkeypatch):
        ...
        # 应有报告目录
        change_reports = openguard_dir / "reports" / "changes" / change_dir.name
    ...
    def test_apply_suite_mode(self, apply_env, monkeypatch):
        ...
        suite_reports = openguard_dir / "reports" / "suites" / "smoke"
