...
class TestDetector:
    def test_unity_detected(self, unity_project: Path):
        from openqa.init.detector import detect_project, PROJECT_TYPE_UNITY, CONFIDENCE_HIGH
    ...
    def test_unity_version_extracted(self, unity_project: Path):
        from openqa.init.detector import detect_project
        result = detect_project(unity_project)
    ...
    def test_web_detected(self, web_project: Path):
        from openqa.init.detector import detect_project, PROJECT_TYPE_WEB
    ...
    def test_backend_detected(self, backend_project: Path):
        from openqa.init.detector import detect_project, PROJECT_TYPE_BACKEND
    ...
    def test_unknown_project(self, tmp_project: Path):
        from openqa.init.detector import detect_project, PROJECT_TYPE_UNKNOWN
    ...
    def test_mixed_type_detected(self, mixed_project: Path):
        from openqa.init.detector import detect_project, PROJECT_TYPE_MIXED
    ...
    def test_mixed_config_yaml_has_sub_projects(self, mixed_project: Path):
        import yaml
        from openqa.init.detector import detect_project
        from openqa.init.config_writer import build_config, write_config
        openqa_dir = mixed_project / "openqa"
        result = detect_project(mixed_project)
        config = build_config(result, ai_hosts=["codebuddy"])
        write_config(config, openqa_dir)
        with (openqa_dir / "config.yaml").open(encoding="utf-8") as f:
...
class TestConfigWriter:
    def test_config_written(self, unity_project: Path):
        ...
        result = detect_project(unity_project)
        config = build_config(result, ai_hosts=["codebuddy"])
        path = write_config(config, unity_project / "openqa")
        assert path.exists()

    def test_config_has_schema_version(self, unity_project: Path):
        from openqa.init.detector import detect_project
        from openqa.init.config_writer import build_config
        result = detect_project(unity_project)
        config = build_config(result, ai_hosts=["codebuddy"])
        assert "schema_version" in config
        assert config["schema_version"].startswith("openqa/config/")
    ...
    def test_config_loadable(self, unity_project: Path):
        from openqa.init.detector import detect_project
        from openqa.init.config_writer import build_config, write_config, load_config
        openqa_dir = unity_project / "openqa"
        result = detect_project(unity_project)
        config = build_config(result, ai_hosts=["codebuddy"])
        write_config(config, openqa_dir)
        loaded = load_config(openqa_dir)
...
class TestIgnoreWriter:
    def test_ignore_written(self, tmp_project: Path):
        from openqa.init.ignore_writer import write_ignore
        path = write_ignore("unity", tmp_project / "openqa")
        assert path.exists()
        assert path.name == "ignore"

    def test_ignore_contains_git(self, tmp_project: Path):
        from openqa.init.ignore_writer import generate_ignore
...
class TestSlashCommands:
    def test_commands_installed_codebuddy(self, unity_project: Path):
        ...
        openqa_dir = unity_project / "openqa"
        openqa_dir.mkdir(exist_ok=True)
        written = install_commands(unity_project, ["codebuddy"], openqa_dir)
        assert "codebuddy" in written
        assert len(written["codebuddy"]) == 4  # new, continue, apply, archive

    def test_canonical_commands_in_openqa(self, unity_project: Path):
        """REQ-02-13：有宿主时命令安装到宿主目录，无宿主时 fallback 到 openqa/commands/。"""
        from openqa.init.slash_commands import install_commands
        openqa_dir = unity_project / "openqa"
        openqa_dir.mkdir(exist_ok=True)
        # 无宿主 → fallback 到 openqa/commands/
        install_commands(unity_project, [], openqa_dir)
    ...
    def test_unknown_host_skipped_gracefully(self, unity_project: Path):
        ...
        openqa_dir = unity_project / "openqa"
        openqa_dir.mkdir(exist_ok=True)
        written = install_commands(unity_project, ["nonexistent_host"], openqa_dir)
        assert "nonexistent_host" not in written


# ──────────────────────────────────────────────────────────────────────────────
# REQ_01-01: openqa init end-to-end (non-interactive)
# ──────────────────────────────────────────────────────────────────────────────
...
class TestInitCommand:
    ...
    def test_init_creates_openqa_dir(self, unity_project: Path):
        self._run_init(unity_project)
        assert (unity_project / "openqa").is_dir()

    def test_init_creates_config_yaml(self, unity_project: Path):
        self._run_init(unity_project)
        assert (unity_project / "openqa" / "config.yaml").is_file()

    def test_init_creates_ignore(self, unity_project: Path):
        self._run_init(unity_project)
        assert (unity_project / "openqa" / "ignore").is_file()
    ...
    def test_init_creates_subdirs(self, unity_project: Path):
        ...
        for sub in ("changes", "artifacts", "reports", "baselines", "knowledge", "commands"):
            assert (unity_project / "openqa" / sub).is_dir(), f"Missing subdir: {sub}"
    ...
    def test_init_web_project(self, web_project: Path):
        ...
        config_path = web_project / "openqa" / "config.yaml"
...
class TestUpdateCommand:
    def test_update_refreshes_commands(self, unity_project: Path, monkeypatch):
        ...
        # Delete one command file
        cmd_file = unity_project / ".codebuddy" / "commands" / "oqa_new.md"
        if cmd_file.exists():
            cmd_file.unlink()
    ...
    def test_update_preserves_changes_dir(self, unity_project: Path, monkeypatch):
        ...
        changes_dir = unity_project / "openqa" / "changes" / "chg-001"
...
class TestHelpCommand:
    ...
    def test_help_suggests_init_when_not_initialized(self, tmp_project: Path, monkeypatch, capsys):
        ...
        run_help(str(tmp_project))
        captured = capsys.readouterr()
        assert "openqa init" in captured.out
...
class TestNonInteractive:
    def test_init_with_profile_flag(self, tmp_project: Path):
        ...
        try:
            ...
            os.chdir(old)
        assert rc == 0
        with (tmp_project / "openqa" / "config.yaml").open(encoding="utf-8") as f:
            cfg = yaml.safe_load(f)
    ...
    def test_interactive_host_selection_by_number(self, unity_project: Path, monkeypatch):
        ...
        import argparse
        from openqa.commands.init import run_init
        from openqa.init.slash_commands import get_supported_hosts
        monkeypatch.chdir(unity_project)
...
class TestReconfigure:
    def test_reconfigure_preserves_changes(self, unity_project: Path, monkeypatch):
        ...
        # Create fake change
        fake_change = unity_project / "openqa" / "changes" / "chg-001"
...
class TestWorkspaceLayout:
    ...
    def test_config_has_suites(self, unity_project: Path):
        ...
        openqa_dir = self._init(unity_project)
        assert (openqa_dir / "suites").is_dir()
        assert (openqa_dir / "suites" / "smoke").is_dir()

    def test_ensure_change_subdirs(self, unity_project: Path):
        """REQ-02-04: each change workspace has test_scripts/ and test_fixtures/."""
        from openqa.workspace.layout import ensure_change_subdirs
        change_dir = unity_project / "openqa" / "changes" / "chg-test"
...
class TestApplySuite:
    ...
    def test_apply_suite_parser_accepts_flag(self):
        ...
        from openqa.cli.main import build_parser
        parser = build_parser()
...
class TestScriptPolicy:
    def test_apply_output_mentions_openqa_only(self, unity_project: Path, monkeypatch, capsys):
        """apply stub output must reference .openqa paths, not project source."""
        import argparse
        from openqa.commands.init import run_init
        from openqa.commands.apply import run_apply
        ...
        # Output should reference openqa/ policy
        assert "openqa/" in out

    def test_slash_command_apply_mentions_script_policy(self):
        """The /oqa:apply command template must mention the script location policy."""
        from openqa.prompts import get_commands
        commands = get_commands()
        apply_body = next(body for name, _, body in commands if name == "oqa:apply")
        assert "openqa/" in apply_body
...
class TestSkillInstallation:
    ...
    def test_skill_fallback_written_to_commands(self, unity_project: Path):
        """REQ-02-13：无宿主时 fallback skill 写入 openqa/commands/"""
        from openqa.init.skills import SKILL_NAMES, install_skills
        openqa_dir = unity_project / "openqa"
        openqa_dir.mkdir(exist_ok=True)
        # 无宿主时触发 fallback
        install_skills(unity_project, [], openqa_dir)
        for skill_name in SKILL_NAMES:
            assert (openqa_dir / "commands" / f"{skill_name}.md").exists(), \
    ...
    def test_skill_installed_to_codebuddy(self, unity_project: Path):
        ...
        from openqa.init.skills import SKILL_NAMES
    ...
    def test_skill_content_has_cli_calls(self, unity_project: Path):
        ...
        self._init(unity_project, hosts=["codebuddy"])
        cli_map = {
            "openqa-new": "openqa new",
            "openqa-continue": "openqa continue",
            "openqa-apply": "openqa apply",
            "openqa-archive": "openqa archive",
        }
    ...
    def test_skill_continue_has_script_generation_guide(self, unity_project: Path):
        """openqa-continue skill 必须包含测试脚本生成指南（REQ-14-05）"""
        self._init(unity_project, hosts=["codebuddy"])
        content = self._skill_content(unity_project, "openqa-continue")
    ...
    def test_skill_apply_has_overlay_guide(self, unity_project: Path):
        """openqa-apply skill 必须包含 overlay 生成指南（REQ-14-06）"""
        self._init(unity_project, hosts=["codebuddy"])
        content = self._skill_content(unity_project, "openqa-apply")
    ...
    def test_skill_mentions_no_direct_app_operation(self, unity_project: Path):
        ...
        for skill_name in SKILL_NAMES:
            content = self._skill_content(unity_project, skill_name)
            if skill_name in ("openqa-apply", "openqa-archive"):
...
class TestUpdateRefreshesSkills:
    def test_update_refreshes_skill_files(self, unity_project: Path, monkeypatch):
        ...
        # 删除一个 skill 文件
        skill_file = unity_project / ".codebuddy" / "skills" / "openqa-new" / "SKILL.md"
...
class TestGitignoreSuggestion:
    ...
    def test_gitignore_contains_openqa_block(self, unity_project: Path):
        """REQ-01-21：.gitignore 包含 OpenQA 规则块。"""
        self._init(unity_project)
        content = (unity_project / ".gitignore").read_text(encoding="utf-8")
        assert "openqa/reports" in content
    ...
    def test_gitignore_idempotent(self, unity_project: Path):
        """重复 init 不应重复追加 OpenQA 块。"""
        self._init(unity_project)
        self._init(unity_project)
        content = (unity_project / ".gitignore").read_text(encoding="utf-8")
        # 只应出现一次 OpenQA 标记
        assert content.count("# ── OpenQA") == 1
    ...
    def test_write_gitignore_suggestion_standalone(self, tmp_path: Path):
        ...
        from openqa.init.ignore_writer import write_gitignore_suggestion
