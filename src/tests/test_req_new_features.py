...
def game_workspace(tmp_path: Path) -> tuple[Path, Path]:
    """返回 (project_root, openguard_dir)，模拟 Unity 游戏项目。"""
...
class TestInitProbe:
    def test_init_probe_returns_structure(self, game_workspace):
        project_root, openguard_dir = game_workspace
        config = yaml.safe_load((openguard_dir / "config.yaml").read_text(encoding="utf-8"))
        from openguard.scan.scanner import run_init_probe
        result = run_init_probe(project_root, openguard_dir, config)
...
class TestKnowledgeScan:
    def test_knowledge_scan_creates_event_catalog(self, game_workspace):
        ...
        from openguard.scan.scanner import run_knowledge_scan
        result = run_knowledge_scan(project_root, openguard_dir, config, "测试登录功能")

        # 检查 event_catalog.yaml 有条目
        event_catalog_path = openguard_dir / "knowledge" / "event_catalog.yaml"
    ...
    def test_knowledge_scan_creates_protocol_catalog(self, game_workspace):
        ...
        from openguard.scan.scanner import run_knowledge_scan
        run_knowledge_scan(project_root, openguard_dir, config, "测试传送功能")

        proto_path = openguard_dir / "knowledge" / "protocol_catalog.yaml"
    ...
    def test_knowledge_scan_skips_fresh_catalog(self, game_workspace):
        ...
        project_root, openguard_dir = game_workspace
        config = yaml.safe_load((openguard_dir / "config.yaml").read_text(encoding="utf-8"))
        from openguard.knowledge.manager import init_knowledge_dir, add_interface_knowledge_item
        init_knowledge_dir(openguard_dir, config)
        ...
        from openguard.commands.new import _run_knowledge_scan
        from openguard.cli.spinner import StepHandle
        handle = StepHandle("Running knowledge-scan", "new")
        _run_knowledge_scan(openguard_dir, "测试目标", handle)
...
class TestInterfaceKnowledgeAnchors:
    def _setup(self, game_workspace):
        ...
        from openguard.knowledge.manager import init_knowledge_dir
        init_knowledge_dir(openguard_dir, config)
        return project_root, openguard_dir, openguard_dir / "knowledge"

    def test_anchor_hash_change_triggers_needs_review(self, game_workspace):
        """REQ-09-14：锚点哈希变化时条目状态降为 needs-review。"""
        _, openguard_dir, knowledge_dir = self._setup(game_workspace)
        from openguard.knowledge.manager import add_interface_knowledge_item, check_interface_knowledge_anchors
    ...
    def test_inferred_items_excluded_from_filter(self, game_workspace):
        ...
        _, openguard_dir, knowledge_dir = self._setup(game_workspace)
        from openguard.knowledge.manager import add_interface_knowledge_item, get_interface_knowledge
...
class TestPreconditions:
    def test_promote_setup_path(self, game_workspace, tmp_path):
        ...
        _, openguard_dir = game_workspace
        script = tmp_path / "login_server.py"
        script.write_text("# login setup\n", encoding="utf-8")

        from openguard.knowledge.preconditions import promote_setup_path
        result = promote_setup_path(
            script, openguard_dir,
            tags=["logged_in"],
            mode="white-box",
            run_id="run-001",
            change_id="chg-001",
    ...
    def test_promote_requires_run_id(self, game_workspace, tmp_path):
        ...
        from openguard.knowledge.preconditions import promote_setup_path
        result = promote_setup_path(script, openguard_dir, tags=["logged_in"], mode="white-box", run_id="")
    ...
    def test_promote_updates_preconditions_yaml(self, game_workspace, tmp_path):
        ...
        from openguard.knowledge.preconditions import promote_setup_path, load_preconditions
        from openguard.knowledge.manager import init_knowledge_dir
        config = yaml.safe_load((openguard_dir / "config.yaml").read_text(encoding="utf-8"))
        init_knowledge_dir(openguard_dir, config)

        promote_setup_path(script, openguard_dir, tags=["logged_in", "map_a"],
                           mode="white-box", run_id="run-1", change_id="c1")

        data = load_preconditions(openguard_dir / "knowledge")
    ...
    def test_find_preconditions_by_tags(self, game_workspace, tmp_path):
        ...
        script = tmp_path / "login.py"
        script.write_text("# login\n", encoding="utf-8")
        from openguard.knowledge.preconditions import promote_setup_path, find_preconditions
        from openguard.knowledge.manager import init_knowledge_dir
        config = yaml.safe_load((openguard_dir / "config.yaml").read_text(encoding="utf-8"))
        init_knowledge_dir(openguard_dir, config)
        ...
        matches = find_preconditions(openguard_dir / "knowledge", ["logged_in"])
    ...
    def test_query_returns_unknown_when_no_match(self, game_workspace):
        ...
        _, openguard_dir = game_workspace
        from openguard.knowledge.preconditions import query_preconditions_for_change
        from openguard.knowledge.manager import init_knowledge_dir
        config = yaml.safe_load((openguard_dir / "config.yaml").read_text(encoding="utf-8"))
        init_knowledge_dir(openguard_dir, config)

        result = query_preconditions_for_change(openguard_dir / "knowledge", ["logged_in", "quest_active"])
    ...
    def test_precondition_mode_recorded(self, game_workspace, tmp_path):
        ...
        promote_setup_path(script, openguard_dir, tags=["logged_in"],
    ...
    def test_setup_path_anchor_check(self, game_workspace, tmp_path):
        ...
        script = tmp_path / "login.py"
        script.write_text("# login\n", encoding="utf-8")
        from openguard.knowledge.preconditions import promote_setup_path, check_setup_path_anchors
        from openguard.knowledge.manager import init_knowledge_dir
        config = yaml.safe_load((openguard_dir / "config.yaml").read_text(encoding="utf-8"))
        init_knowledge_dir(openguard_dir, config)
        ...
        # 模拟符号哈希变化
        reports = check_setup_path_anchors(openguard_dir, current_symbols={"Login.Handle": "newHash"})
        assert any(r["new_status"] == "needs-review" for r in reports)

    def test_setup_paths_dir_created(self, game_workspace):
        """REQ-13-27：init 时创建 test_assets/setup_paths/ 目录。"""
        _, openguard_dir = game_workspace
        assert (openguard_dir / "test_assets" / "setup_paths").is_dir()
...
class TestMatrixPreconditionBinding:
    ...
    def test_task_blocked_when_no_precondition(self, game_workspace, tmp_path):
        ...
        openguard_dir, change_dir, config = self._build_matrix_env(game_workspace, tmp_path)
        from openguard.matrix.generator import _bind_preconditions_to_tasks
        ...
        updated = _bind_preconditions_to_tasks(tasks, openguard_dir)
    ...
    def test_matrix_includes_interface_knowledge_summary(self, game_workspace, tmp_path):
        ...
        matrix = build_test_matrix(change_dir, openguard_dir, config)
...
class TestOpenSpecValidation:
    def test_validate_consistent_spec(self, openspec_project, tmp_path):
        ...
        from openguard.integrations.openspec.detector import validate_openspec_consistency
        change_dir = tmp_path / "chg-001"
        change_dir.mkdir()
        (change_dir / "openspec_link.yaml").write_text(
            yaml.dump({"schema_version": "openguard/openspec_link/v1",
    ...
    def test_new_command_triggers_openspec_validation(self, openspec_project, monkeypatch):
        """REQ-03-11：openguard new --from-openspec 触发有效性校验。"""
        import argparse
...
class TestDirectoryStructure:
    ...
    def test_knowledge_interface_files_initialized(self, game_workspace):
        ...
        _, openguard_dir = game_workspace
        config = yaml.safe_load((openguard_dir / "config.yaml").read_text(encoding="utf-8"))
        from openguard.knowledge.manager import init_knowledge_dir, INTERFACE_KNOWLEDGE_FILES
        init_knowledge_dir(openguard_dir, config)
    ...
    def test_preconditions_yaml_initialized(self, game_workspace):
        ...
        from openguard.knowledge.manager import init_knowledge_dir
        init_knowledge_dir(openguard_dir, config)
        assert (openguard_dir / "knowledge" / "preconditions.yaml").exists()
