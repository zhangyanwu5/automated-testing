...
name: openqa-run
description: >
  当用户输入 /oqa:run 或明确要开始一次测试任务时使用此 Skill。
---

# openqa-run

## 触发条件
用户输入 `/oqa:run <目标>` 或明确表示要开始一次需求/代码修改/缺陷修复对应的测试任务。

## 执行模式
此 Skill 采用**自主驱动模式**：创建 change 后，无需等待用户，持续推进所有产物，
直到遇到 `wait_user` 门槛点或需要人类输入的真正 unknown。
用户说的是 `/oqa:run`，不是 `/oqa:run 然后停下来等待`。
...
在终端执行：
```
openqa run "<目标>" [--from-openspec <id>] [--test-suite <suite>]
```
...
- `stale`/`broken` → 在 `openqa/changes/<id>/test_scripts/` 生成新草稿
- 控制通道选择：
  - `web`/`h5`：Playwright
...
description: >
  ...
  正常流程中 /oqa:run 会自动驱动循环，无需手动触发此命令。
---

# openqa-continue

## 触发条件
用户输入 `/oqa:continue` 或要求恢复/查看当前进度。
正常流程中 `/oqa:run` 自动驱动循环，仅在中断后手动恢复时使用此命令。
...
| `agent_action` | 行动 |
| --- | --- |
| `run_cli` | 再次执行 `openqa continue`，然后循环 |
| `generate` | 生成 `missing_artifacts` 中列出的缺失产物，写入后再执行 `openqa continue`，然后循环 |
...
- 已存在且新鲜的产物不得重复生成。
- 不引用 `stale`/`broken` 脚本。
- 脚本只写入 `openqa/changes/<id>/test_scripts/`，不写入项目源码。
...
name: openqa-apply
description: >
  当用户输入 /oqa:apply 或要求执行测试时使用此 Skill。
...
## 触发条件
用户输入 `/oqa:apply` 或要求执行代码 Review 和测试。
...
name: openqa-archive
description: >
  当用户输入 /oqa:archive 或要求归档当前 QA change 时使用此 Skill。
---

# openqa-archive

## 触发条件
用户输入 `/oqa:archive` 或要求关闭已完成的 change。
