...
ADVANCE_STEPS: list[tuple[str, str, str, bool]] = [
    ...
    (
        "test_knowledge.md",
        "测试知识",
        "基于 requirements.md 和 impact_graph.json，生成 test_knowledge.md（见 openqa-run skill 中的 Cases 契约格式）。",
        False,
    ),
    (
        "review_plan.md",
...
UNKNOWNS_HINT = (
    ...
    "  解决所有 unknowns 后重新运行 `openqa _advance`。"
)

READY_FOR_APPLY = (
    "所有必需产物已就绪。\n"
    "\n"
    "停止：向用户呈现以下摘要，等待用户确认后才能执行 `openqa apply`："
)

MISSING_ARTIFACT_HEADER = "Agent，请："

AUTO_GENERATE_FAILED = "  请手动解决后重新运行 `openqa _advance`"
