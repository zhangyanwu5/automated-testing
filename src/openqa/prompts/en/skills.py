...
Skill files are installed to .codebuddy/skills/openqa-<name>/SKILL.md
...
# ──────────────────────────────────────────────────────────────────────────────
# Skill content — one per /oqa:* command
# ──────────────────────────────────────────────────────────────────────────────
...
name: openqa-run
description: >
  Use this skill when the user invokes /oqa:run or asks to start a QA test
...
## When to use
Trigger when user says `/oqa:run <goal>` or asks to begin a test task.
...
The user said `/oqa:run`, not `/oqa:run then stop and wait`.
...
Run in terminal:
```
openqa run "<goal>" [--from-openspec <id>] [--test-suite <suite>]
```
...
- Running `openqa _advance` (just run it)
...
- `stale`/`broken` → generate new draft in `openqa/changes/<id>/test_scripts/`
...
- Write scripts only inside `openqa/`. Never write to project source directories.
...
description: >
  ...
  loop runs automatically after /oqa:run — you do not need to invoke this.
---

# openqa-continue

## When to use
Trigger when user says `/oqa:continue` or asks to resume/check progress.
In normal flow, `/oqa:run` drives the loop automatically. Use this only for
...
| `agent_action` | What to do |
| --- | --- |
| `run_cli` | Run `openqa continue` again, then loop |
| `generate` | Generate the missing artifact(s) listed in `missing_artifacts`, write them, then run `openqa continue` again and loop |
...
**Never stop for:**
- Generating a file (just generate it)
- Running `openqa continue` again (just run it)
...
- Write scripts only to `openqa/changes/<id>/test_scripts/`.
...
name: openqa-apply
description: >
  Use this skill when the user invokes /oqa:apply or asks to run code Review
...
## When to use
Trigger when user says `/oqa:apply` or asks to execute tests.
...
## Guardrails
- Do not operate the target application directly (REQ-14-07). Execution is handled by the OpenQA executor, not directly by the AI.
...
name: openqa-archive
description: >
  Use this skill when the user invokes /oqa:archive or asks to archive the
...
## When to use
Trigger when user says `/oqa:archive` or asks to close a completed change.
...
_SKILL_CONTENT_MAP: dict[str, str] = {
    "openqa-run":      SKILL_RUN,
    "openqa-continue": SKILL_CONTINUE,
    "openqa-apply":    SKILL_APPLY,
    "openqa-archive":  SKILL_ARCHIVE,
}
