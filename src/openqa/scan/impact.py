...
def build_impact_graph(
    ...
    # 受影响基线
    affected_baselines = _find_affected_baselines(openqa_dir, changed_files)

    graph: dict[str, Any] = {
        "schema_version": "openqa/impact_graph/v1",
...
def run_impact_analysis(
    ...
    graph = build_impact_graph(change_dir, openqa_dir, config)
...
def _find_affected_baselines(
    ...
    baselines_dir = openqa_dir / "baselines"
    ...
    for meta_file in sorted(baselines_dir.glob("**/*.yaml"))[:20]:
        ...
        results.append({
            ...
            "path": str(meta_file.relative_to(openqa_dir)),
        })
