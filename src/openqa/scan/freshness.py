...
from openqa.scan.scanner import ANALYZER_VERSION, _file_hash
...
def check_freshness(
    ...
    anchor_reports = _check_test_assets_anchors(openqa_dir, snapshot)
    if anchor_reports:
    ...
    result: dict[str, Any] = {
        "schema_version": "openqa/freshness/v1",
        "checked_at": _now_iso(),
        "is_fresh": is_fresh,
        "checks": checks,
...
def run_freshness_check(change_dir: Path, openqa_dir: Path) -> tuple[bool, Path]:
...
def _check_test_assets_anchors(
    ...
    return check_anchor_freshness(openqa_dir, current_symbols, current_req_fps)
