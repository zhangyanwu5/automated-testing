...
from openqa.tools.fs.hash import file_hash as _file_hash
from openqa.tools.fs.ignore import load_ignore as _load_ignore_tool, is_ignored as _is_ignored_tool
from openqa.tools.analysis.symbols import detect_language as _detect_language, extract_symbols as _extract_symbols

# 扫描器版本：升级时应同步更新，触发新鲜度失效（REQ-04-06）
ANALYZER_VERSION = "openqa-scanner/0.1.0"
...
def load_ignore_patterns(openqa_dir: Path) -> list[str]:
    """从 openqa/ignore 加载忽略 glob 模式列表（委托给 tools）。"""
    return _load_ignore_tool(openqa_dir)
...
def _check_test_asset_anchors(
    ...
    for meta_file in sorted(scripts_dir.glob("**/*.meta.yaml")):
        ...
        results.append({
            "meta_file": str(meta_file.relative_to(openqa_dir)),
...
def run_full_scan(
    ...
    ignore_patterns = load_ignore_patterns(openqa_dir)
    ...
    anchor_checks = _check_test_asset_anchors(openqa_dir, file_records)
...
def run_incremental_scan(
    ...
    current = run_full_scan(project_root, openqa_dir, config)
    ...
    delta: dict[str, Any] = {
        "schema_version": "openqa/delta/v1",
        "analyzer_version": ANALYZER_VERSION,
        "scanned_at": _now_iso(),
        "added": added,
        "modified": modified,
...
def write_full_index(openqa_dir: Path, scan_data: dict[str, Any]) -> Path:
    """写入全量扫描索引 full_index.json（REQ-04 全量扫描产物）。"""
    full_index: dict[str, Any] = {
        "schema_version": "openqa/full_index/v1",
        "created_at": scan_data.get("scanned_at", _now_iso()),
        "analyzer_version": scan_data.get("analyzer_version", ANALYZER_VERSION),
    ...
    path = openqa_dir / "artifacts" / "full_index.json"
...
def execute_scan(
    ...
    if use_full:
        scan_data = run_full_scan(project_root, openqa_dir, config)
        snapshot_path = write_snapshot(
            change_dir, scan_data, requirements_fp, change_id, "full", project_root
        )
        write_full_index(openqa_dir, scan_data)
        return snapshot_path, None
    else:
        scan_data, delta = run_incremental_scan(
            project_root, openqa_dir, config, existing_snapshot
        )
        snapshot_path = write_snapshot(
...
def run_init_probe(
    ...
    probe_result: dict[str, Any] = {
        "schema_version": "openqa/init_probe/v1",
        "probed_at": _now_iso(),
        "project_type": project_type,
        "structure": structure,
...
def run_knowledge_scan(
    ...
    from openqa.knowledge.manager import add_interface_knowledge_item, INTERFACE_KNOWLEDGE_FILES
    catalog_map = {
    ...
    result: dict[str, Any] = {
        "schema_version": "openqa/knowledge_scan/v1",
        "scanned_at": _now_iso(),
        "project_type": project_type,
