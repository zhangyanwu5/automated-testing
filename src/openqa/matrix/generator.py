...
MATRIX_SCHEMA_VERSION = "openqa/test_matrix/v1"
...
def build_test_matrix(
    ...
    # 加载稳定脚本（REQ-05-11）
    script_refs = _load_script_references(openqa_dir, suite)
    ...
    interface_knowledge_summary = _query_interface_knowledge(openqa_dir)

    # 为每个任务绑定前置路径（REQ-05-13）
    tasks = _bind_preconditions_to_tasks(tasks, openqa_dir)
...
def run_matrix_generation(
    ...
    matrix = build_test_matrix(change_dir, openqa_dir, config, test_suite)
...
def _bind_preconditions_to_tasks(
    ...
    """
    try:
        from openqa.knowledge.preconditions import find_preconditions
...
def _query_interface_knowledge(
    ...
    try:
        from openqa.knowledge.manager import get_interface_knowledge, INTERFACE_KNOWLEDGE_FILES
        catalog_map = {
