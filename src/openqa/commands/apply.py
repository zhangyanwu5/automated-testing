"""``openqa apply`` — 执行代码 Review 与测试矩阵（REQ-07）。
...
def run_apply(args: argparse.Namespace) -> int:
    ...
    if suite:
        return _run_suite_mode(args, openqa_dir, config, suite, gate)
    return _run_change_mode(args, openqa_dir, config, change_id_arg, gate, test_suite, review_level)
...
def _run_suite_mode(
    ...
    suite_dir = openqa_dir / "suites" / suite_name
    if not suite_dir.is_dir():
        available = _list_suites(openqa_dir)
        print(f"Error: suite '{suite_name}' not found. Available suites: {available}", file=sys.stderr)
        return 1

    print(f"openqa apply --suite {suite_name}")
...
def _list_suites(openqa_dir: Path) -> str:
