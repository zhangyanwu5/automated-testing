...
from openqa.init.detector import (
    detect_project,
    DetectionResult,
    CONFIDENCE_HIGH,
    CONFIDENCE_MEDIUM,
...
from openqa.init.ignore_writer import write_ignore, write_gitignore_suggestion
from openqa.init.slash_commands import (
    install_commands,
    detect_installed_hosts,
    get_supported_hosts,
)
from openqa.init.skills import install_skills
from openqa.workspace.layout import get_openqa_dir, ensure_subdirs, is_initialized
...
def _select_language(yes: bool, forced_lang: str | None) -> str:
    ...
    返回语言代码（如 "zh" 或 "en"）。
    """
    from openqa.prompts import list_languages
    ...
    from openqa.cli.prompt import singleselect
        ...
        for provider in registry.get_all_spec_providers():
            if hasattr(provider, "detect_hint_for_init"):
                ...
                if info.get("installed"):
                    ...
                    print(f"  Use `openqa new --from-{provider.name} <change-id>` to link QA changes.")
        print()
    except Exception:
        pass
...
def _resolve_hosts(args_hosts: list[str] | None, project_root: Path, yes: bool) -> list[str]:
    ...
    from openqa.cli.prompt import multiselect
    from openqa.prompts import get_cli
...
def run_init(args: argparse.Namespace) -> int:
    from openqa.cli.spinner import step, ok, warn, set_plain_mode
    from openqa.prompts import get_cli

    project_root = Path.cwd()
    openqa_dir = get_openqa_dir(project_root)
    ...
    if is_initialized(project_root) and not getattr(args, "reconfigure", False):
        ...
        from openqa import prompts as _prompts_mod
        _prompts_mod.set_language(_early_lang)
        print(get_cli("ALREADY_INIT_MSG").format(openqa_dir=openqa_dir))
        return 0
    ...
    ok(get_cli("CREATED_WORKSPACE").format(path=openqa_dir), cmd="init")
    ...
    ignore_path = write_ignore(project_type, openqa_dir)
