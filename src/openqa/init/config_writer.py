"""从 DetectionResult 生成 openqa/config.yaml。
...
from openqa.init.detector import DetectionResult, CONFIDENCE_HIGH, CONFIDENCE_MEDIUM
...
class _OpenQADumper(yaml.Dumper):
    pass


_OpenQADumper.add_representer(str, _represent_str_literal)


def _dump(data: Any) -> str:
    return yaml.dump(
        data,
        Dumper=_OpenQADumper,
...
def write_config(config: dict[str, Any], openqa_dir: Path) -> Path:
    """将 config.yaml 写入 *openqa_dir*，返回写入路径。"""
    openqa_dir.mkdir(parents=True, exist_ok=True)
    config_path = openqa_dir / "config.yaml"
    config_path.write_text(_dump(config), encoding="utf-8")
    return config_path


def load_config(openqa_dir: Path) -> dict[str, Any]:
    """加载并返回 config.yaml 字典，文件不存在时抛出 FileNotFoundError。"""
    config_path = openqa_dir / "config.yaml"
    if not config_path.exists():
        raise FileNotFoundError(
            f"未找到 openqa/config.yaml：{config_path}。请先运行 `openqa init`。"
        )
...
def _scan_paths_for_type(project_type: str) -> tuple[list[str], list[str]]:
    ...
    excludes_common = [
        "node_modules/", ".git/", "openqa/", "openspec/",
        "*.lock", "*.log",
    ]
