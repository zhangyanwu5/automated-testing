...
def write_ignore(project_type: str, openqa_dir: Path) -> Path:
    """写入 ignore 文件并返回路径。"""
    openqa_dir.mkdir(parents=True, exist_ok=True)
    ignore_path = openqa_dir / "ignore"
...
def generate_gitignore_block() -> str:
    """返回建议追加到 .gitignore 的 OpenQA 规则块（REQ-01-21）。"""
    return _GITIGNORE_BLOCK


def write_gitignore_suggestion(project_root: Path) -> tuple[bool, str]:
    """在项目根目录 .gitignore 中追加 OpenQA 建议规则（REQ-01-21 / REQ-02-14）。

    - 若 .gitignore 已包含 OpenQA 块，则跳过（幂等）。
    ...
    if gitignore_path.exists():
        ...
        if marker in existing:
            return False, ".gitignore 中已包含 OpenQA 规则，跳过。"
        # 追加到末尾
        with gitignore_path.open("a", encoding="utf-8") as f:
            f.write("\n" + block)
        return True, f"已将 OpenQA 规则追加到 {gitignore_path}"
    else:
        gitignore_path.write_text(block, encoding="utf-8")
        return True, f"已创建 {gitignore_path} 并写入 OpenQA 规则"
