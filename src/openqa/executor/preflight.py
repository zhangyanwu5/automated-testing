...
def pre_apply_check(
    ...
    if change_dir and change_dir.is_dir():
        ...
        freshness = check_freshness(change_dir, openqa_dir)
    ...
    script_check = _check_scripts(change_dir, openqa_dir, suite_name, gate)
    ...
    # 5. 侵入策略合规校验（REQ-13-06~09）
    intrusion_check = _check_intrusion(config, openqa_dir)
    ...
    # 6. OpenSpec 变化检测（REQ-12-07）
    if change_dir and change_dir.is_dir():
        openspec_check = _check_openspec_changes(change_dir, openqa_dir)
    ...
    return {
        "schema_version": "openqa/pre_apply_check/v1",
        "checked_at": _now_iso(),
...
def _check_scripts(
    ...
    if suite_name:
        ...
        scripts = list_suite_scripts(openqa_dir, suite_name, gate)
...
def _check_intrusion(config: dict[str, Any], openqa_dir: Path) -> dict[str, Any]:
    """校验侵入策略合规性（REQ-13-06~09）。"""
    checks: list[dict[str, Any]] = []
    blocked = False

    from openqa.assets.lifecycle import check_intrusion_strategy, check_intrusion_strategy_changed
    ...
    if check_intrusion_strategy_changed(openqa_dir, config):
        checks.append(_warn(
...
def _check_openspec_changes(change_dir: Path, openqa_dir: Path) -> dict[str, Any]:
