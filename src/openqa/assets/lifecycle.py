...
def get_meta_path(openqa_dir: Path, script_name: str) -> Path:
    """根据脚本文件名返回对应的 .meta.yaml 路径。"""
    stem = Path(script_name).stem
    return openqa_dir / "test_assets" / "scripts" / f"{stem}.meta.yaml"
...
def load_suite(suite_path: Path) -> dict[str, Any]:
    ...
    if not suite_path.exists():
        return {
            "schema_version": "openqa/suite/v1",
...
def list_suite_scripts(
    ...
    for entry in suite.get("scripts", []):
        ...
        meta_path = get_meta_path(openqa_dir, script_name)
        meta = read_meta(meta_path)
...
def update_suites_after_promote(
    ...
    for suite_name in suite_membership:
        add_script_to_suite(openqa_dir, suite_name, script_path)
...
def check_intrusion_strategy_changed(
    ...
    cache_path = openqa_dir / ".intrusion_strategy_sig"
...
def build_suite_execution_manifest(
    ...
    return {
        "schema_version": "openqa/suite_manifest/v1",
