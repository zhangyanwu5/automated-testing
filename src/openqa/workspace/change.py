...
每次 `openqa run` 都会在 openqa/changes/<change-id>/ 下创建完整工作区。
...
# next_cli 值
NEXT_CLI_ADVANCE = "openqa _advance"
NEXT_CLI_APPLY   = "openqa apply"

# 向后兼容：next_step 建议值
NEXT_CONTINUE = NEXT_CLI_ADVANCE
NEXT_APPLY    = NEXT_CLI_APPLY
NEXT_ARCHIVE  = "openqa archive"
...
def _write_intent(
    ...
    from openqa.prompts import get_artifact_fn
...
def _write_snapshot(
    ...
    snapshot: dict[str, Any] = {
        ...
        "note": "骨架快照，等待 `openqa continue` 扫描器填充。",
    }
...
def _write_artifact_index(
    ...
    只记录 change 工作区内（openqa/changes/<id>/）的产物。
    执行报告（run_report.*、events.jsonl 等）位于 openqa/reports/changes/<id>/
    ...
    index: dict[str, Any] = {
        "schema_version": "openqa/artifact_index/v1",
        "change_id": change_id,
        "created_at": created_at,
        "updated_at": created_at,
        "artifacts": artifacts,
        # 执行报告通过 openqa/reports/changes/<id>/change_run_index.yaml 单独索引
        "reports_dir": f"openqa/reports/changes/{change_id}/",
    }
    if from_openspec:
...
def list_active_changes(openqa_dir: Path) -> list[Path]:
...
def latest_change(openqa_dir: Path) -> Path | None:
