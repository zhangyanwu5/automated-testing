...
目录：
  openqa/knowledge/preconditions.yaml   — 已验证前置路径按状态标签索引
  openqa/test_assets/setup_paths/       — 前置路径脚本 + .meta.yaml 锚点
...
def promote_setup_path(
    ...
    *,
    ...
    setup_paths_dir = openqa_dir / "test_assets" / "setup_paths"
    ...
    meta: dict[str, Any] = {
        "schema_version": "openqa/setup_path_meta/v1",
    ...
    _upsert_precondition(
        knowledge_dir,
        tags=tags,
        setup_path=f"openqa/test_assets/setup_paths/{dest.name}",
        mode=mode,
        run_id=run_id,
    )
