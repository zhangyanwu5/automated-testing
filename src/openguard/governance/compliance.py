...
设计边界（REQ-10）：
  - OpenGuard 工具进程不调用大模型
  - 测试脚本只写入 openguard/ 目录
  - 默认只读 OpenSpec，不修改
"""
...
def check_script_path_boundary(script_path: str | Path, openguard_dir: Path) -> bool:
    """检查脚本路径是否在 openguard/ 内（REQ-01-11 / REQ-10）。"""
    script = Path(script_path).resolve()
    openguard = openguard_dir.resolve()
    return str(script).startswith(str(openguard))
