...
Strings printed to stdout for AI Agent consumption during `openguard _advance`.
"""
from __future__ import annotations

# 状态机步骤列表（供 openguard _advance 内部使用）
...
ADVANCE_STEPS: list[tuple[str, str, str, bool]] = [
    ...
    (
        ...
        (
            ...
            "(see Cases contract format in openguard-run skill)."
        ),
        False,
    ),
    (
...
UNKNOWNS_HINT = (
    ...
    "  Re-run `openguard _advance` after all unknowns are resolved."
)

READY_FOR_APPLY = (
    "All required artifacts are ready.\n"
    "\n"
    "STOP: Present the following summary to the user and wait for confirmation before running `openguard apply`:"
)

MISSING_ARTIFACT_HEADER = "Agent, please:"

AUTO_GENERATE_FAILED = "  Please resolve manually then re-run `openguard _advance`"
