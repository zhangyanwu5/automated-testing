...
def init_knowledge_dir(openguard_dir: Path, config: dict[str, Any]) -> None:
    """初始化 knowledge/ 目录，写入各文件骨架（REQ-09-08）。

    `openguard init` 生成的项目画像可作为 project_profile.yaml 的初始来源，
    ...
    _ensure_yaml(knowledge_dir / "project_profile.yaml", {
        "schema_version": "openguard/knowledge/v1",
...
def promote_script(
    ...
    *,
    ...
    from openguard.workspace.asset_lifecycle import build_full_meta
    ...
    from openguard.workspace.asset_lifecycle import update_suites_after_promote
...
def check_flaky_and_demote(
    ...
    meta_path = openguard_dir / "test_assets" / "scripts" / f"{Path(script_name).stem}.meta.yaml"
    ...
    if is_flaky:
        ...
        _record_flaky_in_knowledge(openguard_dir, script_name, run_results, old_status)
...
def get_flaky_history(openguard_dir: Path, script_name: str) -> list[dict[str, Any]]:
    """读取脚本的 flaky 历史记录。"""
    meta_path = openguard_dir / "test_assets" / "scripts" / f"{Path(script_name).stem}.meta.yaml"
    if not meta_path.exists():
        return []
...
def should_promote_script(
    ...
    *,
    ...
    # 检查 flaky 历史
    flaky_history = get_flaky_history(openguard_dir, script_name)
    if flaky_history:
