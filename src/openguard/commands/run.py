"""``openguard run <target>`` — 开始一次 QA 测试任务（REQ-03-01）。

取代原 `openguard new`，合并了创建 change 工作区和触发 AI 驱动循环的职责。

执行步骤：
1. 校验工作区已初始化。
2. 生成 change-id 并创建 openguard/changes/<change-id>/ 目录，写入骨架产物。
3. 若 --from-openspec 则写入 openspec_link.yaml 并做有效性校验。
4. 执行 knowledge-scan（REQ-03-09）。
5. 查询 preconditions.yaml（REQ-03-10）。
6. 将 state.yaml 的 agent_action 设为 run_cli、next_cli 设为 openguard _advance，
   触发 AI 驱动循环自主推进准备阶段。
7. 打印摘要，告知 AI 进入驱动循环。
"""
