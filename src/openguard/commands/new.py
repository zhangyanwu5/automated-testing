"""``openguard new <target>`` — 开始一次新的 QA change（REQ-03-01 / REQ-03-02）。

执行步骤：
1. 校验工作区已初始化。
2. 生成 change-id 并创建 openguard/changes/<change-id>/ 目录。
...
def run_new(args: argparse.Namespace) -> int:
    ...
    if from_openspec:
        with step("Validating OpenSpec consistency", cmd="new") as s:
            _run_openspec_validation(change_dir, openguard_dir, s)

    # knowledge-scan
    with step("Running knowledge-scan", cmd="new") as s:
        _run_knowledge_scan(openguard_dir, target, s)

    # preconditions 查询
    _query_preconditions(change_dir, openguard_dir)
    ...
    print()
    print(f"agent_action : {state.get('agent_action', 'run_cli')}")
    print(f"next_cli     : {state.get('next_cli', 'openguard _advance')}")
    print()
...
def _run_openspec_validation(change_dir, openguard_dir, s=None) -> None:
...
def _run_knowledge_scan(openguard_dir, target: str, s=None) -> None:
    ...
    try:
        ...
        result = run_knowledge_scan(project_root, openguard_dir, config, target)
...
def _query_preconditions(change_dir, openguard_dir) -> None:
    ...
    若 test_knowledge.md 尚未生成（openguard new 刚执行，Agent 还未 continue），
    则跳过此步骤，等 Agent 填写后由 openguard continue 阶段再执行。
    """
    try:
...
def _append_unknown(change_dir, entry: str) -> None:
    ...
    if unknowns_path.exists():
        ...
        current = "# Unknowns\n\n<!-- 待确认项，由 openguard run/_advance 自动填充。 -->\n\n"
