"""``openguard archive`` — 归档已完成 change，沉淀知识（REQ-03-05/07/08 / REQ-09）。
...
def run_archive(args: argparse.Namespace) -> int:
    ...
    # ── 归档前检查 ────────────────────────────────────────────────────────────
    issues = _pre_archive_check(change_dir, openguard_dir)
    if issues["blockers"]:
    ...
    # ── 可晋升脚本列表（REQ-03-07）────────────────────────────────────────────
    promotable = _find_promotable_scripts(change_dir, openguard_dir)
    if promotable:
    ...
    if dry_run:
        print("dry-run complete. Run `openguard archive` to perform the real archive.")
        return 0
    ...
    run_report = _load_latest_run_report(change_dir, openguard_dir)
    ...
    state["phase"] = PHASE_ARCHIVED
    state["agent_action"] = "done"
    state["next_cli"] = None
...
def _pre_archive_check(change_dir: Path, openguard_dir: Path) -> dict[str, Any]:
    """归档前检查（REQ-03-05/06）。"""
    blockers: list[str] = []
    warnings: list[str] = []
    ...
    change_report_dir = openguard_dir / "reports" / "changes" / change_dir.name
    if not change_report_dir.is_dir():
        warnings.append("No execution report directory found; suggest running `openguard apply` first")
    else:
        run_dirs = [d for d in change_report_dir.iterdir()
                    if d.is_dir() and d.name.startswith("run-")]
        if not run_dirs:
            warnings.append("No execution records found; suggest running `openguard apply` first")
...
def _load_latest_run_report(change_dir: Path, openguard_dir: Path) -> dict[str, Any] | None:
    """加载该 change 最近一次执行报告。"""
    change_report_dir = openguard_dir / "reports" / "changes" / change_dir.name
    if not change_report_dir.is_dir():
        return None
