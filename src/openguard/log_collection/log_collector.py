...
def generate_log_enhancement_suggestions(
    ...
    md_lines = [
        "# 日志增强建议",
        "",
        "<!-- 由 OpenGuard 自动生成，由开发者决定是否采纳。OpenGuard 不会自动修改项目代码（REQ-13-11）。 -->",
        "",
    ]
...
def generate_debug_build_config(
    ...
    不修改主分支代码；debug 配置存放在 openguard/artifacts/debug_build_config/，
    与生产配置隔离，测试完成后不需要手动还原（因为主分支未被修改）。
    """
    debug_dir = openguard_dir / "artifacts" / "debug_build_config"
    debug_dir.mkdir(parents=True, exist_ok=True)

    config: dict[str, Any] = {
        "schema_version": "openguard/debug_build_config/v1",
    ...
    if project_type == "unity":
        config["unity_debug_settings"] = {
            ...
            "build_target": "debug",
            "define_symbols": ["OPENGUARD_DEBUG", "QA_VERBOSE_LOGGING"],
        }
    elif project_type in ("unreal", "unreal-game"):
