...
def get_run_dir(
    ...
    *,
    ...
    """返回本次执行的报告目录路径（不创建）。"""
    if suite_name:
        return openguard_dir / "reports" / "suites" / suite_name / run_id
    elif change_id:
        return openguard_dir / "reports" / "changes" / change_id / run_id
    else:
...
def ensure_run_dir(
    ...
    *,
    ...
    run_dir = get_run_dir(openguard_dir, run_id, change_id=change_id, suite_name=suite_name)
...
def update_suite_index(
    ...
    suite_dir = openguard_dir / "reports" / "suites" / suite_name
...
def update_change_run_index(
    ...
    change_report_dir = openguard_dir / "reports" / "changes" / change_id
