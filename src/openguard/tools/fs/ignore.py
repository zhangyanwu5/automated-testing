...
@tool(
    name="fs.load_ignore",
    description="Read openguard/ignore file and return a list of glob patterns.",
    input_schema={"openguard_dir": {"type": "string"}},
    output_schema={"type": "array", "items": {"type": "string"}},
    tags=["fs", "ignore"],
)
def load_ignore(openguard_dir: str | Path) -> list[str]:
    """读取 openguard/ignore 文件，返回 glob 模式列表（过滤注释和空行）。"""
    ignore_file = Path(openguard_dir) / "ignore"
